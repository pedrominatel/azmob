...
public class DriverDataExchange {
	...
	// GetObject
	public GetResult GetObject(IClientConnection connection, ObisCode obis,
			int classId, int attributeId) {
		...
		try {
			List<GetResult> getResults = connection.get(HIGH_TIMEOUT, true,
					getObject); // set the timeout with more than 15s for big
								// data i.e Events
	...
	public GetResult GetObjectWithSelector(IClientConnection connection,
			ObisCode obis, int classId, int attributeId,
			SelectiveAccessDescription selector) {
		// Get parameter to read
		GetRequest getObject = new GetRequest(classId, obis, attributeId);
	...
	// GetObjectWithSelector
	public GetResult GetObjectWithSelector(IClientConnection connection,
			ObisCode obis, int classId, int attributeId, Data struct,
			int accessSelector) {

		// Get parameter to read
		GetRequest getObject = new GetRequest(classId, obis, attributeId);
		getObject.setAccessSelection(new SelectiveAccessDescription(
	...
	// SetObject
	public AccessResultCode SetObjectXML(IClientConnection connection,
			String xmlFilePath) {
	...
	// SetObject
	public AccessResultCode SetObject(IClientConnection connection,
			ObisCode obis, int classId, int attributeId, SetRequest setObject) {
	...
	// SetDateTimeObject
	public AccessResultCode SetDateTime(IClientConnection connection,
			ObisCode obis, int classId, int attributeId) {
	...
	// SetDateTimeObject
	public AccessResultCode SetDateTime(IClientConnection connection,
			ObisCode obis, int classId, int attributeId, byte[] dateOctet) {
	...
	// Action
	public MethodResult Action(IClientConnection connection, ObisCode obis,
			int classId, int attributeId, int data) {
		// Action
