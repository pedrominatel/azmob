...
import java.util.*;
	 ...
	 * <pre>
	 *   for (MyService service : CustomServiceLoader<MyService>.load(MyService.class)) {
	...
	public final class CustomServiceLoader<S> implements Iterable<S> {
	    ...
	    private CustomServiceLoader(Class<S> service, ClassLoader classLoader) {
	     ...
	     * @return a new CustomServiceLoader
	     */
	    public static <S> CustomServiceLoader<S> load(Class<S> service, ClassLoader classLoader) {
	        if (classLoader == null) {
	            classLoader = ClassLoader.getSystemClassLoader();
	        }
	        return new CustomServiceLoader<S>(service, classLoader);
	    }

	    private void internalLoad() {
	        services.clear();
	        try {
	            String name = "assets/services/" + service.getName();
	    ...
	    public static <S> CustomServiceLoader<S> load(Class<S> service) {
	        return CustomServiceLoader.load(service, Thread.currentThread().getContextClassLoader());
	    }
	    ...
	    public String toString() {
	        return "CustomServiceLoader for " + service.getName();
	    }
	    ...
	    private class ServiceIterator implements Iterator<S> {
	        ...
	        public ServiceIterator(CustomServiceLoader<S> sl) {
	        ...
	        public void closeQuietly(BufferedReader closeable) {
	        	if (closeable != null) {
	        	try {
	        	closeable.close();
